...
export function TechnicianLanguageSettings({ technicians }: TechnicianLanguageSettingsProps) {
    ...
    return (
        <div className="bg-white shadow rounded-lg p-6 max-w-md">
            <h3 className="text-lg font-medium text-gray-900 mb-4">{t.settings.techLanguage}</h3>
            <div className="space-y-4">
                ...
                {technicians.length === 0 && (
                    <p className="text-sm text-gray-500 text-center py-4">{t.settings.noTechnicians}</p>
                )}
            </div>
        </div>
    );
